...
view: +input_data {
  derived_table: {
    sql:  SELECT
          ...
          (CAST(TIMESTAMP(FORMAT_TIMESTAMP('%F %H:%M:%E*S', order_items.created_at , 'America/Los_Angeles')) AS DATE)) AS order_items_created_date,
          ...
          COUNT(DISTINCT CASE WHEN order_items.returned_at IS NOT NULL  THEN order_items.id  ELSE NULL END) AS order_items_returned_count,
          COUNT(*) AS order_items_count,
          COALESCE(SUM(CASE WHEN order_items.returned_at IS NOT NULL  THEN order_items.sale_price  ELSE NULL END), 0) AS order_items_returned_total_sale_price
