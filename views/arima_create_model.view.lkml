    ...
    create_process: {
      ...
      sql_step: CREATE OR REPLACE VIEW  @{looker_temp_dataset_name}.{% parameter model_name.select_model_name %}_arima_detect_anomalies_{{ _explore._name }}
                AS SELECT {% parameter arima_training_data.select_time_column %} as time_series_timestamp
                  ...
                  ,upper_bound
                FROM
                  ML.DETECT_ANOMALIES(MODEL  @{looker_temp_dataset_name}.{% parameter model_name.select_model_name %}_arima_model_{{ _explore._name }},STRUCT({% parameter arima_training_data.set_anomaly_prob_threshold %} AS anomaly_prob_threshold))
     ;;
    }
  }
