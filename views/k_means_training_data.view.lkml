view: k_means_training_data {
  ...
  derived_table: {
    ...
    sql_create: CREATE OR REPLACE VIEW looker_pdts.{% parameter model_name.select_model_name %}_training_data
                  AS  SELECT
                        ...
                        {% assign features = _filters['select_features'] | sql_quote | replace: '"','' | remove: "'" %}
                          {{ features }}
                      FROM bqml_training_data.{% parameter select_training_data %}
    ;;
  }
